...
JSON schema identifiers mark the record family/version. This first release uses validated provider functions and generated-fixture regression tests rather than a separate schema compiler. Changes to output contracts need tests and a version decision.
...
- `track_map.py`: bounded signal evidence, pulse alternatives, local tonal content and uncertainty. No automatic edit follows a detector's score.
...
- `transition_lab.py`: exact excerpt comparisons, new native candidates for the supported media-only shift, render attachment and feedback. It does not operate an open Live session or independently certify who performed an export.
- `cli.py` and `mcp_server.py`: the common tool surface.
