...
> **In brief.** Pocket keeps MIDI as exact, versioned **material**. You can
> import a MIDI file or a clip from a saved Live set, generate simple patterns
> (always alongside a "leave it alone" option), make precise edits with locks on
> what must not change, organize phrases and sections, and export a standard
> MIDI file that lists anything it couldn't carry. Related tools cover control
> curves, per-note expression, sound plans and hypotheses drawn from audio.
> Pocket never decides key, groove or taste for you.
>
> **You need** nothing extra for most of it. MIDI file import and export need
> the `midi` extra (`python -m pip install -e '.[midi]'`). A few steps work
> inside Ableton Live under supervision; [what's supported](midi-capabilities.md)
> lists them.
...
This implementation provides offline MIDI material, deterministic alternatives, guarded edits, file curves, sound plans, and a supervised native-candidate evidence pipeline. One narrow synthetic stock-instrument workflow has been saved/reopened, rendered, technically promoted and reopened after relocation. General native MIDI fidelity failed specific probes, native Serum control is deferred, and no human musical approval is claimed. [What's supported](midi-capabilities.md) separates what works today from what's planned.
...
A separate supervised Live 12.4.5 stock Utility experiment saved and reopened exact ramp, step and pulse envelopes. Its parameter-specific override/re-enable sequence and one-point edit were observed, and the clean pulse copy was rendered. Those observations qualify that UI experiment only. The production candidate profile still excludes Utility automation; public native envelope writing, gesture capture and the Serum macro test remain unavailable.
...
`serum_inspect`, `serum_presets` and `serum_plan` specialize those public functions. Serum 1/2 and their FX identities remain distinct; no private preset structure is edited. **Native Serum control is deferred.** No license, install, live parameter write, preset load/save, complete-patch inspection, FX route or MPE receiver has been qualified.
...
This abandonment path has been exercised on a real failed preparation. The released package passes three-note insertion, stale refusal with zero dispatch, a single velocity change, and exact note readback after saving and reopening. Separate source preservation and sealing checks remain necessary. Prepared-only cancellation exists internally but is not publicly exposed until its native acceptance gate passes.
...
These providers read exact passages of local recordings and keep every result as a hypothesis, with its alternatives and abstentions, until someone authors a correction. They need no model unless you choose one of the optional learned adapters.
...
An inspected qualified model reuses its retained seven-case qualification only after the current executable, checkpoint and runtime profile match; the requested crop still runs twice. Inline input performs qualification in the same call. The model child process has a 60-second limit; a call that exceeds it publishes no result. Any change to the runner's bytes requires fresh qualification. The seven-case qualification and direct, background, CLI and stdio-MCP inference produce identical artifacts in the recorded environment. These bounded successes do not guarantee a deadline under other loads, musical accuracy or another runtime profile. Retained-artifact query/correction and mocked protocol tests remain separate evidence.
...
musical acceptance on real recordings (milestone E8). The ten broader synthetic
path cases are separate execution evidence.
