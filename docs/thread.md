...
with coordinate evidence, or `status: unknown` with a reason. MCP exposes these functions as
`thread_source_position` and `thread_arrangement_position`. Positions outside
...
CLI `thread` and MCP `thread` return the compact summary by default.
`thread-region`, `thread-find-clips` and `thread-export` accept a JSON file containing that
summary or its bare handle. In MCP, pass the handle object directly to
`thread_region`, `thread_find_clips` or `thread_export`. The four MCP summary/query
tools return one compact JSON text block; parse it without expecting a duplicate
`structuredContent` record. For the old raw CLI output, use `thread --full`.

The full `inspect_set` library call above remains compatible. For normal agent
work, use `pocket_music.thread_queries`:
...
receipt = export_thread(handle, "full-inventory.json")
```
...
- `export_thread(handle, output_path)`
...
cover partial edge samples. One-ULP arithmetic handling matches Peek.
