...
Workshop explores several possible orders from a sealed [record bag](record-bag.md).
...
```python
from pocket_music.set_workshop import plan_set_routes, record_plan_feedback, replan_set
...
`duration` ledger. Transitions retain On Deck's reasons, unknowns, tempo options,
