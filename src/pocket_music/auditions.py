...
def _measure_audio(source, plan):
    ...
    try:
        with sf.SoundFile(source) as audio:
            if audio.format not in {"WAV", "WAVEX"} or audio.subtype not in {"FLOAT", "DOUBLE"}:
...
def audition_feedback(store_root: str, request_id: str, attachment: ArtifactHandle,
    ...
    def work():
        ...
        start, end = (_integer(v, "interval frame") for v in interval_frames)
        if end <= start or end > evidence["signal"]["frames"]:
            raise PocketError("Feedback interval is outside this exact render")
        _text(actor, "actor")
        _text(note, "note", 8000)
