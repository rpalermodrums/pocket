"""Bounded queries over immutable, local saved-set snapshots.
...
from .set_map import _filesystem_version, inspect_set, source_position
...
def inspect_set_summary(path: str | Path, *, cache_dir: str | Path | None = None,
    ...
    return {'schema': 'pocket.set-summary/v1', 'handle': handle,
            ...
            'raw_state': 'Use export_set_map(handle, output_path) for the full inventory', 'scope': LIMITATION}
...
def export_set_map(handle: SetHandle, output_path: str | Path) -> dict:
