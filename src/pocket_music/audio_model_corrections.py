...
COVERAGE = {'provider': 'audio-model-hypothesis-corrections-v1', 'execution': 'retained_evidence_only',
...
def _load(handle, store_root):
    ...
    from .audio_pulse_hypotheses import load_pulse_hypotheses
    ...
    chain.reverse()
    initial = load_pulse_hypotheses(chain[0][0], store_root)
...
def query(*, store_root: str, hypotheses: ArtifactHandle,
    ...
    if view == 'annotations':
        ...
        rows = [{'source': initial['source'], 'analysis': initial['analysis'], 'model': initial['model'],
